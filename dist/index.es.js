...
class Route {
  ...
  get parameterSegments() {
    ...
    return (_this$template$match$ = (_this$template$match = this.template.match(/{[^}?]+\??}/g)) == null ? void 0 : _this$template$match.map(segment => ({
  ...
  matchesUrl(url) {
    ...
    const pattern = this.template.replace(/\/{[^}?]*\?}/g, '(\/[^/?]+)?') // TODO: the above line with the leading slash is necessary to pick up completely optional *segments*,
  ...
  compile(params) {
    ...
    return this.template.replace(/{([^}?]+)\??}/g, (_, segment) => {
      ...
      if ([null, undefined].includes(params[segment]) && this.parameterSegments.find(({
        name
...
class Router extends String {
  ...
  toString() {
    ...
    const unhandled = Object.keys(this._params).filter(key => !this._route.parameterSegments.some(({
      name
    }) => name === key)).filter(key => key !== '_query').reduce((result, current) => _extends({}, result, {
  ...
  current(name, params) {
    const url = this._config.absolute ? this._location().host + this._location().pathname : this._location().pathname.replace(this._config.url.replace(/^\w*:\/\/[^/]+/, ''), '').replace(/^\/+/, '/'); // Find the first route that matches the current URL

    const [current, route] = Object.entries(this._config.routes).find(([_, route]) => new Route(name, route, this._config).matchesUrl(url)) || [undefined, undefined]; // If a name wasn't passed, return the name of the current route
    ...
    const routeParams = this._dehydrate(route); // If the current window URL has no route parameters, and the passed parameters are empty, return true


    if (Object.values(params).every(p => !p) && !Object.values(routeParams).length) return true; // Check that all passed parameters match their values in the current window URL
  ...
  _parse(params = {}, route = this._route) {
    ...
    if (Array.isArray(params)) {
      ...
      params = params.reduce((result, current, i) => !!segments[i] ? _extends({}, result, {
  ...
  _defaults(route) {
    return route.parameterSegments.filter(({
      name
    }) => this._config.defaults[name]).reduce((result, {
      name
    }, i) => _extends({}, result, {
  ...
  _substituteBindings(params, {
    ...
    return Object.entries(params).reduce((result, [key, value]) => {
      ...
      if (!value || typeof value !== 'object' || Array.isArray(value) || !parameterSegments.some(({
        name
      }) => name === key)) {
  ...
  _dehydrate(route) {
    ...
    let pathname = this._location().pathname // If this Laravel app is in a subdirectory, trim the subdirectory from the path
    ...
    const dehydrate = (hydrated, template = '', delimiter) => {
      ...
      return segments.reduce((result, current, i) => {
        ...
        return /{[^}?]+\??}/.test(current) && values[i] ? _extends({}, result, {
          [current.replace(/.*{|\??}.*/g, '')]: values[i].replace(current.match(/^[^{]*/g), '').replace(current.match(/[^}]*$/g), '')
        }) : result;
      }, {});
    };
