...
var Route = /*#__PURE__*/function () {
  ...
  function Route(name, definition, config) {
    var _definition$bindings;

    this.name = name;
    this.definition = definition;
    this.bindings = (_definition$bindings = definition.bindings) != null ? _definition$bindings : {};
    this.config = config;
  }
  /**
   ...
   */
  _proto.matchesUrl = function matchesUrl(url) {
    ...
    var pattern = this.template.replace(/\/{[^}?]*\?}/g, '(\/[^/?]+)?') // TODO: the above line with the leading slash is necessary to pick up completely optional *segments*,
  ...
  _proto.compile = function compile(params) {
    var _this = this;

    if (!this.parameterSegments.length) return this.template;
    return this.template.replace(/{([^}?]+)\??}/g, function (_, segment) {
      var _params$segment;

      // If the parameter is missing but is not optional, throw an error
      if ([null, undefined].includes(params[segment]) && _this.parameterSegments.find(function (_ref) {
        var name = _ref.name;
        return name === segment;
      }).required) {
        throw new Error("Ziggy error: '" + segment + "' parameter is required for route '" + _this.name + "'.");
      }

      return encodeURIComponent((_params$segment = params[segment]) != null ? _params$segment : '');
    }).replace(/\/+$/, '');
  };
  ...
  _createClass(Route, [{
    ...
    get: function get() {
      ...
      var origin = !this.config.absolute ? '' : this.definition.domain ? "" + this.config.url.match(/^\w+:\/\//)[0] + this.definition.domain + (this.config.port ? ":" + this.config.port : '') : this.config.url;
    ...
    get: function get() {
      ...
      return (_this$template$match$ = (_this$template$match = this.template.match(/{[^}?]+\??}/g)) == null ? void 0 : _this$template$match.map(function (segment) {
        return {
          name: segment.replace(/{|\??}/g, ''),
          required: !/\?}$/.test(segment)
        };
      })) != null ? _this$template$match$ : [];
    }
  }]);
...
var Router = /*#__PURE__*/function (_String) {
   ...
   * @param {Object} config - Ziggy configuration.
   */
  function Router(name, params, absolute, config) {
    var _this;
    ...
    _this = _String.call(this) || this;
    _this._config = config != null ? config : typeof Ziggy !== 'undefined' ? Ziggy : globalThis == null ? void 0 : globalThis.Ziggy;
    _this._config = _extends({}, _this._config, {
      absolute: absolute
    });
    ...
    if (name) {
      ...
      _this._route = new Route(name, _this._config.routes[name], _this._config);
  ...
  _proto.toString = function toString() {
    ...
    var unhandled = Object.keys(this._params).filter(function (key) {
      ...
      return _extends({}, result, (_extends2 = {}, _extends2[current] = _this2._params[current], _extends2));
    }, {});
    return this._route.compile(this._params) + lib.stringify(_extends({}, unhandled, this._params['_query']), {
      addQueryPrefix: true,
      arrayFormat: 'indices',
  ...
  _proto.current = function current(name, params) {
    var _this3 = this;

    var url = this._config.absolute ? this._location().host + this._location().pathname : this._location().pathname.replace(this._config.url.replace(/^\w*:\/\/[^/]+/, ''), '').replace(/^\/+/, '/'); // Find the first route that matches the current URL

    var _ref2 = Object.entries(this._config.routes).find(function (_ref3) {
      var route = _ref3[1];
      return new Route(name, route, _this3._config).matchesUrl(url);
    }) || [undefined, undefined],
        current = _ref2[0],
        route = _ref2[1]; // If a name wasn't passed, return the name of the current route
    ...
    var match = new RegExp("^" + name.replace(/\./g, '\\.').replace(/\*/g, '.*') + "$").test(current);
    if ([null, undefined].includes(params) || !match) return match;
    var routeObject = new Route(current, route, this._config);
    params = this._parse(params, routeObject);

    var routeParams = this._dehydrate(route); // If the current window URL has no route parameters, and the passed parameters are empty, return true
    ...
    return Object.entries(params).every(function (_ref4) {
      var key = _ref4[0],
          value = _ref4[1];
  ...
  _proto._location = function _location() {
    ...
    var _ref5 = typeof window !== 'undefined' ? window.location : {},
        _ref5$host = _ref5.host,
        host = _ref5$host === void 0 ? '' : _ref5$host,
        _ref5$pathname = _ref5.pathname,
        pathname = _ref5$pathname === void 0 ? '' : _ref5$pathname,
        _ref5$search = _ref5.search,
        search = _ref5$search === void 0 ? '' : _ref5$search;

    return {
      host: (_this$_config$locatio = (_this$_config$locatio2 = this._config.location) == null ? void 0 : _this$_config$locatio2.host) != null ? _this$_config$locatio : host,
      pathname: (_this$_config$locatio3 = (_this$_config$locatio4 = this._config.location) == null ? void 0 : _this$_config$locatio4.pathname) != null ? _this$_config$locatio3 : pathname,
      search: (_this$_config$locatio5 = (_this$_config$locatio6 = this._config.location) == null ? void 0 : _this$_config$locatio6.search) != null ? _this$_config$locatio5 : search
    };
  }
  /**
  ...
  _proto._parse = function _parse(params, route) {
    var _this4 = this;
    ...
    var segments = route.parameterSegments.filter(function (_ref6) {
      var name = _ref6.name;
    ...
    if (Array.isArray(params)) {
      ...
      params = params.reduce(function (result, current, i) {
        ...
        return !!segments[i] ? _extends({}, result, (_extends3 = {}, _extends3[segments[i].name] = current, _extends3)) : _extends({}, result, (_extends4 = {}, _extends4[current] = '', _extends4));
      }, {});
      ...
      params = (_params = {}, _params[segments[0].name] = params, _params);
    }

    return _extends({}, this._defaults(route), this._substituteBindings(params, route));
  }
  /**
  ...
  _proto._defaults = function _defaults(route) {
    var _this5 = this;

    return route.parameterSegments.filter(function (_ref7) {
      var name = _ref7.name;
      return _this5._config.defaults[name];
    }).reduce(function (result, _ref8, i) {
      var _extends5;

      var name = _ref8.name;
      return _extends({}, result, (_extends5 = {}, _extends5[name] = _this5._config.defaults[name], _extends5));
    }, {});
  }
  /**
  ...
  _proto._substituteBindings = function _substituteBindings(params, _ref9) {
    var bindings = _ref9.bindings,
        parameterSegments = _ref9.parameterSegments;
    return Object.entries(params).reduce(function (result, _ref10) {
      var _extends7;
      ...
      if (!value || typeof value !== 'object' || Array.isArray(value) || !parameterSegments.some(function (_ref11) {
        var name = _ref11.name;
        return name === key;
      })) {
        var _extends6;

        return _extends({}, result, (_extends6 = {}, _extends6[key] = value, _extends6));
      }
      ...
      return _extends({}, result, (_extends7 = {}, _extends7[key] = value[bindings[key]], _extends7));
    }, {});
  }
  /**
  ...
  _proto._dehydrate = function _dehydrate(route) {
    var _this$_location$searc;

    var pathname = this._location().pathname // If this Laravel app is in a subdirectory, trim the subdirectory from the path
    .replace(this._config.url.replace(/^\w*:\/\/[^/]+/, ''), '').replace(/^\/+/, ''); // Given part of a valid 'hydrated' URL containing all its parameter values,
    ...
    var dehydrate = function dehydrate(hydrated, template, delimiter) {
      if (template === void 0) {
        template = '';
      }

      var _map = [hydrated, template].map(function (s) {
        return s.split(delimiter);
      }),
          values = _map[0],
          segments = _map[1];

      return segments.reduce(function (result, current, i) {
        var _extends8;

        // Only include template segments that are route parameters
        // AND have a value present in the passed hydrated string
        return /{[^}?]+\??}/.test(current) && values[i] ? _extends({}, result, (_extends8 = {}, _extends8[current.replace(/.*{|\??}.*/g, '')] = values[i].replace(current.match(/^[^{]*/g), '').replace(current.match(/[^}]*$/g), ''), _extends8)) : result;
      }, {});
    };

    return _extends({}, dehydrate(this._location().host, route.domain, '.'), dehydrate(pathname, route.uri, '/'), lib.parse((_this$_location$searc = this._location().search) == null ? void 0 : _this$_location$searc.replace(/^\?/, '')));
  };
...
var _route = function route(name, params, absolute, config) {
  var router = new Router(name, params, absolute, config);
  return name ? router.toString() : router;
};
var ZiggyVue = {
  install: function install(v, options) {
    return v.mixin({
      methods: {
        route: function route(name, params, absolute, config) {
          if (config === void 0) {
            config = options;
          }
...
var useRoute = function useRoute(defaultConfig) {
...
export { ZiggyVue, _route as route, useRoute };
